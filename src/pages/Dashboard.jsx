...
const Dashboard = () => {
  const { leads, fetchLeads, isLoading } = useLeads();

  // ── Fetch leads from the API on mount ─────────────────────────────────
  // We request a generous limit so the dashboard stats and recent leads are
  // populated from real data. The leads page has its own independent fetch.
  useEffect(() => {
    fetchLeads({ limit: 100, sortBy: 'createdAt', sortOrder: 'desc' });
  }, [fetchLeads]);

  // ── Derive stats from the loaded leads array ──────────────────────────
  // These match what the backend's getLeadStats() aggregation returns,
  // computed here for instant display without a second API call.
      ...
      <div className="max-w-7xl mx-auto space-y-6">
        ...
        {isLoading && leads.length === 0 && (
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
            {[...Array(4)].map((_, i) => (
              <div key={i} className="h-28 bg-white dark:bg-gray-800 rounded-xl animate-pulse border border-slate-200 dark:border-gray-700" />
            ))}
          </div>
        )}
